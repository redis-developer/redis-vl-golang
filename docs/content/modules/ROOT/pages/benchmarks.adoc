...
Only Docker is required: each benchmark run starts its own throwaway
Redis testcontainer (pinned `redis:8.8.0`, override with `REDIS_IMAGE`)
and tears it down afterwards, so neither language benefits from a
warmed-up instance left behind by the other. From the repository root:
...
To benchmark against an external Redis instead (for example a remote
server, to include network serialization), pass `-url` / `--url`
explicitly.
